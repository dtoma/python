...
class TestBTorrentTokenizer(unittest.TestCase):
    ...
    def test_tokenize_list(self, ints_and_or_bytes):
        ...
        """
        expected_content = itertools.chain.from_iterable(
            tokenize(item) for item in ints_and_or_bytes
        )
        self.assertEqual(
            list(tokenize(f"l{''.join(ints_and_or_bytes)}e")),
            ["l", *expected_content, "e"],
        )
